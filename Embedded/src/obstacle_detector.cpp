...
void ObstacleDetector::setConfig(const ObstacleDetectionConfig& config) {
    ...
    config_ = config;
            std::cout << "ObstacleDetector: Configuration updated" << std::endl;
}
...
void ObstacleDetector::updateData() {
    ...
    try {
        ...
        std::cerr << "ObstacleDetector: Data update error: " << e.what() << std::endl;
    }
}
...
float ObstacleDetector::estimateDistanceFromAcceleration() {
    ...
    float dt = (current.timestamp - previous.timestamp) / 1000.0f;
    if (dt <= 0) return 0.0f;
