...
class MotorDriver:
    ...
    def __init__(self, pwm):
        ...
        self.pwm.setPWMFreq(self.config.MOTOR_PWM_FREQUENCY)
    ...
    def MotorRun(self, motor, index, speed):
        if speed > self.config.MOTOR_MAX_SPEED:
            speed = self.config.MOTOR_MAX_SPEED
        elif speed < 0:
            speed = 0

        if motor == 0:
            if index == 1:
                self.pwm.setPWM(self.AIN1, 0, self.config.MOTOR_PWM_RESOLUTION)
                self.pwm.setPWM(self.AIN2, 0, 0)
                self.pwm.setPWM(self.PWMA, 0, int(speed * self.config.MOTOR_SPEED_MULTIPLIER))
            else:
                self.pwm.setPWM(self.AIN1, 0, 0)
                self.pwm.setPWM(self.AIN2, 0, self.config.MOTOR_PWM_RESOLUTION)
                self.pwm.setPWM(self.PWMA, 0, int(speed * self.config.MOTOR_SPEED_MULTIPLIER))
        else:
            if index == 1:
                self.pwm.setPWM(self.BIN1, 0, self.config.MOTOR_PWM_RESOLUTION)
                self.pwm.setPWM(self.BIN2, 0, 0)
                self.pwm.setPWM(self.PWMB, 0, int(speed * self.config.MOTOR_SPEED_MULTIPLIER))
            else:
                self.pwm.setPWM(self.BIN1, 0, 0)
                self.pwm.setPWM(self.BIN2, 0, self.config.MOTOR_PWM_RESOLUTION)
                self.pwm.setPWM(self.PWMB, 0, int(speed * self.config.MOTOR_SPEED_MULTIPLIER))
